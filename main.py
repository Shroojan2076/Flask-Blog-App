...
class Comment(db.Model):
    ...
    post_id: Mapped[str] = mapped_column(String(1000), db.ForeignKey('blog_posts.id'))
...
def register():
    ...
    if form.validate_on_submit():
        ...
        if user and current_user.is_authenticated:
            flash('You are already signed up with that email. Login instead.', 'warning')
...
def login():
    ...
    if form.validate_on_submit():
        ...
        if user:
            ...
            flash('The email is not registered, please register first.', 'danger')
...
def show_post(post_id):
    ...
    if form.validate_on_submit():
        if current_user.is_authenticated:
            ...
            flash('You can comment only after you log in.', 'danger')
            return redirect(url_for('login'))
